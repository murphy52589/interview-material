...
const languages = (acceptLanguage, possibleLanguages) => {
    ...
    // iterating through splitLanguages and only returning languages that include a specific language
    return splitLanguages.filter((lang) => possibleLanguages.includes(lang));
}

const acceptLang = "en-US, fr-CA, fr-FR";
const possLanguages = ["fr-FR", "en-US"];
const expectedOutput = [ "en-US", "fr-FR"];
const unexpectedOutput = ['fr-FR', "en-US", "fr-CA"];
const output = languages(acceptLang, possLanguages);
console.log(output);
// expected output
console.assert(JSON.stringify(expectedOutput) === JSON.stringify(output));
// expected to not match
console.log(JSON.stringify(unexpectedOutput) === JSON.stringify(output));
// expected not to be reverse
console.log(JSON.stringify(possLanguages) === JSON.stringify(output));
